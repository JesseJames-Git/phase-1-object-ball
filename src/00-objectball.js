...
function numPointsScored (playerName){
    ...
    for(const team of [game.home, game.away]){
        if(team.players[playerName]){
            return `Points: ${team.players[playerName].points}`
        }
    }
}
...
function shoeSize (playerName){
    ...
    for(const team of [game.home, game.away]){
        if (team.players[playerName]){
            return `Shoe Size : ${team.players[playerName].shoe}`
        }
    }
}
        ...
        for (const playerName in team.players) {
            const player = team.players[playerName];
                if (player.shoe > largestShoe) {
                    ...
                    bigFootPlayer = playerName;
                }
            }
        }

          return bigFootPlayer;
        }
function bigShoeRebounds(){
    const game = gameObject()
    let largestShoePlayer = bigShoePlayer()
    
      for (const team of [game.home, game.away]) {
        const playerStats = team.players[largestShoePlayer];
        if (playerStats) {
